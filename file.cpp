...
void bitmap_sync(struct current_partition *part, uint32_t bit_idx,
                 uint8_t btmp_type) {
  uint32_t off_sec = bit_idx / 4096;  // 本i结点索引相对于位图的扇区偏移量
  uint32_t off_size =
      off_sec * BLOCK_SIZE;  // 本i结点索引相对于位图的字节偏移量
  uint32_t sec_lba;
...
int32_t file_read(struct file *file, void *buf, uint32_t count) {
  ...
  uint32_t *all_blocks =
      (uint32_t *)malloc(BLOCK_SIZE + 48);  // 用来记录文件所有的块地址
  if (all_blocks == NULL) {
  ...
  if (!(block_read_start_idx < 139 && block_read_end_idx < 139)) {
    cout << "ERROR:!(block_read_start_idx < 139 && block_read_end_idx < 139)"
         << endl;
    return FAIL;
  }
  ...
  if (read_blocks == 0) {  // 在同一扇区内读数据,不涉及到跨扇区读取
    ...
    if (block_read_end_idx < 12) {  // 待读的数据在12个直接块之内
      ...
      read_blocks_from_disk_unsafe(cur_part->v_disk, indirect_block_table,
    ...
    if (block_read_end_idx < 12) {  // 数据结束所在的块属于直接块
      ...
      if (!(file->fd_inode->i_block[12] != 0)) {  // 确保已经分配了一级间接块表
      ...
      /* 再将间接块地址写入all_blocks */
      indirect_block_table = file->fd_inode->i_block[12];
      read_blocks_from_disk_unsafe(
          cur_part->v_disk, indirect_block_table, (char *)all_blocks + 12,
...
int32_t file_write(struct file *file, const void *buf, uint32_t count) {
  if ((file->fd_inode->i_size + count) >
    ...
    cout << "exceed max file_size 71680 bytes, write file failed\n";
    return -1;
  }
  uint8_t *io_buf = (uint8_t *)malloc(BLOCK_SIZE);
  if (io_buf == NULL) {
    cout << "file_write: sys_malloc for io_buf failed\n";
    return -1;
  }
  uint32_t *all_blocks =
      (uint32_t *)malloc(BLOCK_SIZE + 48);  // 用来记录文件所有的块地址
  if (all_blocks == NULL) {
    cout << "file_write: sys_malloc for all_blocks failed\n";
    return -1;
  }

  const uint8_t *src = (uint8_t *)buf;  // 用src指向buf中待写入的数据
  ...
  if (file->fd_inode->i_block[0] == 0) {
    block_lba = block_bitmap_alloc(cur_part);
    if (block_lba == -1) {
      cout << "file_write: block_bitmap_alloc failed\n";
      return -1;
    }
    file->fd_inode->i_block[0] = block_lba;
  ...
  if (!(file_will_use_blocks <= 140)) return -1;
  ...
  if (add_blocks == 0) {
    ...
    if (file_has_used_blocks <= 12) {  // 文件数据量将在12块之内
      ...
      if (!(file->fd_inode->i_block[12] != 0)) return -1;
      indirect_block_table = file->fd_inode->i_block[12];
      read_blocks_from_disk_unsafe(cur_part->v_disk, indirect_block_table,
                                   (char *)(all_blocks + 12), 1);
    }
    ...
    if (file_will_use_blocks <= 12) {
      ...
      if (!(file->fd_inode->i_block[block_idx] != 0)) return -1;
      all_blocks[block_idx] = file->fd_inode->i_block[block_idx];
      ...
      while (block_idx < file_will_use_blocks) {
        block_lba = block_bitmap_alloc(cur_part);
        if (block_lba == -1) {
          cout << "file_write: block_bitmap_alloc for situation 1 failed\n";
          return -1;
        }
      ...
      block_lba = block_bitmap_alloc(cur_part);
      if (block_lba == -1) {
        cout << "file_write: block_bitmap_alloc for situation 2 failed\n";
        return -1;
      }

      if (!(file->fd_inode->i_block[12] == 0))
        return -1;  // 确保一级间接块表未分配
      /* 分配一级间接块索引表 */
      indirect_block_table = file->fd_inode->i_block[12] = block_lba;
      ...
      while (block_idx < file_will_use_blocks) {
        block_lba = block_bitmap_alloc(cur_part);
        if (block_lba == -1) {
          cout << "file_write: block_bitmap_alloc for situation 3 failed\n";
          return -1;
        }
        all_blocks[block_idx++] = block_lba;
  ...
  while (bytes_written < count) {  // 直到写完所有数据
    ...
    if (first_write_block) {
      read_blocks_from_disk_unsafe(cur_part->v_disk, sec_lba, (char *)io_buf,
                                   1);
      first_write_block = false;
    }
    memcpy(io_buf + sec_off_bytes, src, chunk_size);
    write_blocks_to_disk(cur_part->v_disk, sec_lba, (char *)io_buf, 1);
